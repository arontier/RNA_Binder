...
def main(args):
    ...
    loss.eval()
    print(f"args: \n {args}")
    print(f"args.valid_subset: {args.valid_subset}")

    try:
        task.load_dataset(args.data, combine=False, epoch=1)
        dataset = task.dataset(args.data)
    except KeyError:
        raise Exception("Cannot find dataset: " + args.data)
